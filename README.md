...
Orchestra DSL powers the swarm's business automation pipeline with `.orc` workflow definitions:

- **`lead_import_enrich_score.orc`** — Import leads → deduplicate → enrich → score → CRM sync (conditional on score > 80)
- **`email_outbound.orc`** — Draft → review → approval gate → compliance gate → send via SendGrid → CRM sync

These workflows leverage 72 task routes across 18 agent castes, 4 pipeline gates (approval, compliance, dedup, metrics), and external services via [calculus-tools](https://github.com/financecommander/calculus-tools) (SendGrid, GHL, Slack, Twilio, Stripe, etc.).
...
| **[BUNNY](https://github.com/financecommander/BUNNY)** | Security — AI Guardian Agent, edge worker runtime |

...
class TestLLMProvider:
    ...
    def test_llm_provider_execute(self):
        ...
        provider = LLMProvider()
        agent = Agent(name="test_agent", provider="llm", system_prompt="You are helpful")
        task = Task(name="task1", description="Generate a summary", inputs={"text": "Sample text"})
        context = Context()
    ...
    def test_llm_provider_build_prompt(self):
        ...
        provider = LLMProvider()
        agent = Agent(name="test", provider="llm", system_prompt="System message")
        task = Task(name="task1", description="Task description", inputs={"input": "value"})
        context = Context()
    ...
    def test_llm_provider_repr(self):
        ...
        provider = LLMProvider(config={"model": "gpt-3.5-turbo", "provider_type": "openai"})
